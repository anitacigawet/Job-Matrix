...
/**
 * Checks the deterministic showroom in a real browser and overwrites the three
 * README screenshots. The default workflow does not contact the hosted
 * showroom.
 *
 * First build and serve the showroom locally on port 3000:
 *   pnpm build:showroom
 *   pnpm exec playwright install chromium
 *   pnpm exec vite preview --host 127.0.0.1 --port 3000 --strictPort
 *
 * Then, from a second terminal, run:
 *   pnpm showroom:capture
 *
 * Set JOB_MATRIX_SCREENSHOT_URL only when you deliberately want a different
 * address. Successful completion confirms the seven main routes, the guarded
 * application flow, the fictional Gmail flow, zero browser errors, and zero
 * external requests beyond the selected showroom origin.
 */
