...
async function assembleRelease(name, kind, sqlJsRoot, nodeRuntime = null) {
  ...
  const copyMap = [
    ...
    [path.join(projectRoot, "packaging", "start.sh"), path.join(releaseRoot, "start.sh")],
    [path.join(projectRoot, "README.md"), path.join(releaseRoot, "README.md")],
    ...
    [path.join(projectRoot, "docs", "images"), path.join(releaseRoot, "docs", "images")],
    [path.join(projectRoot, "docs", "screenshots"), path.join(releaseRoot, "docs", "screenshots")],
  ...
  await chmod(path.join(releaseRoot, "start.sh"), 0o755);
