...
## [Unreleased] — Hosted product transition (2026-08-16)

### Removed

- Retired the experimental NotebookLM feature set: generated podcasts,
  briefings, infographics/slides, prompt catalog, authentication bridge,
  scheduler, media serving, settings, navigation, and per-job actions.
- Removed NotebookLM from Python setup and runtime health checks.

### Changed

- Established the hosted, account-based product direction while keeping the
  current unauthenticated development server loopback-only.
- Kept résumé text as optional context for guided applications rather than an
  input to generated media.
- Preserved the final NotebookLM edition at Git commit `742ed69` and left
  existing local database rows and media untouched.
...
- Made the JobSpy foundation explicit in the README and added its upstream MIT
  copyright and license to `THIRD_PARTY_NOTICES.md`. Job Matrix remains under
  PolyForm Noncommercial 1.0.0 and makes no ScootSolute LLC ownership claim.
...
- `shared/work-style-suggestions.ts` — four categories (Introvert-friendly · Extrovert-friendly · Hands-on / physical · Entry-level / no degree) × three levels (Low / Medium / High) × eight curated titles per level, plus a one-line description per level framed by _the work_, not the person. Titles chosen for real search-engine hits on Indeed / Adzuna / LinkedIn — no whimsical entries.
