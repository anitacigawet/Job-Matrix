...
| `/settings` | LLM provider, notifications, auto-scan, NotebookLM auth |
| `/briefings` | NotebookLM-powered briefings — audio, infographic, text outputs |
...
| `go-to-settings-global` | Open the Settings page. Surfaced in the global TopNav, the dashboard's missing-key warning banner, the briefings empty state, and the Landing page. |
...
| `go-to-briefings` | Global TopNav → `/briefings` (also used by DailyBriefingStrip on the dashboard) |
...
| `generate-daily-briefing` | DailyBriefingStrip on the dashboard — queue a new Daily Coach Audio briefing when none exists or the latest is stale. |
| `retry-daily-briefing` | DailyBriefingStrip — re-queue the Daily Coach briefing after a failed generation. |
| `play-daily-briefing` | DailyBriefingStrip — open the latest Daily Coach Audio in `/briefings` (the strip is a surface, not a player). |
...
| `open-briefing-menu-{id}` | Open the per-job briefing dropdown on an applied-job card |
| `generate-pre-application-brief-{id}` | Generate a Pre-Application Brief for a specific applied job |
| `generate-interview-prep-{id}` | Generate Interview Prep audio for a specific applied job |
| `generate-interview-flashcards-{id}` | Generate Interview Flashcards for a specific applied job |
| `generate-interview-quiz-{id}` | Generate an Interview Quiz for a specific applied job |
| `open-briefings-from-job-{id}` | Navigate to `/briefings` from a job's briefing menu |
...
| `save-auto-scan` | Save the auto-scan schedule (also persists `auto-daily-briefing` and `auto-weekly-briefing` toggles) |
...
### NotebookLM auth (on `/settings`)

| Hook | Purpose |
| ---- | ------- |
| `refresh-notebooklm-auth` | Force a fresh probe of NotebookLM session-cookie status |
| `start-notebooklm-login` | Spawn the `notebooklm login` subprocess (opens a browser) |
| `confirm-notebooklm-login` | Confirm the browser sign-in completed — saves the session cookies |

### Briefings (`/briefings`)

| Hook | Purpose |
| ---- | ------- |
| `generate-briefing-{type}` | Kick off generation of a briefing of the selected type (e.g. `generate-briefing-daily_coach_audio`). Returns immediately; backend runs the long Python subprocess in the background and the row's status flips when done. |
| `view-briefing-{id}` | Expand/collapse a specific briefing's detail view (audio player, image, or text content) |
| `delete-briefing-{id}` | Remove a briefing and its media file |
| `toggle-briefing-audio-{id}` | Play/pause the custom audio player for a briefing. Player is keyboard-operable (Space / K to toggle, ←/→ to seek 5s, focusable container). |
| `set-playback-rate-{rate}` | Select a playback speed inside an audio player. `{rate}` is one of `1`, `1.25`, `1.5`, `2`. Rendered as a radio-group of pill buttons inside each `briefing-audio-{id}` container — scope-disambiguate by walking from the parent. |
| `download-briefing-audio-{id}` | Download the MP4 audio file for a completed audio briefing. |
| `clear-briefing-search` | Clear the briefing-inbox search box (X inside the input). |
| `clear-briefing-filters` | Reset the briefing inbox: clear search, set type and status filters back to "all", and reset the sort to "newest". Rendered both inline in the filter bar (when anything is active) and inside the `no-matching-briefings` empty state. |
...
| `daily-briefing-strip` | The Daily Coach Audio surface at the top of the dashboard main column. Four mutually-exclusive sub-states (empty / generating / failed / fresh) — read the inner `.ds-title` + badges to disambiguate. |
...
| `resume-text-section` | The Résumé Text section (promoted out of Skills & Experience in D11.16a so the Resume Critique briefing has a discoverable input). |
...
### NotebookLM + Briefings state

| Hook | Meaning |
| ---- | ------- |
| `notebooklm-auth-card` | Container on the Settings page |
| `notebooklm-auth-status` | Badge — "Connected" \| "Expired" \| "Not signed in" \| "Unknown" |
| `notebooklm-awaiting-signin` | Amber hint shown while a sign-in subprocess window is open |
| `notebooklm-posix-instructions` | Cyan hint shown on POSIX hosts with the manual command to run |
| `notebooklm-recent-activity` | Activity summary block: 7-day briefing counts + recent failures |
...
| `loading-briefings` | Briefings list query is in flight |
| `empty-briefings` | No briefings have ever been generated |
| `briefings-list` | Container for the list of briefings |
| `filtered-briefings-count` | Live-region text reading "Showing X of Y" — only present when at least one filter or non-default sort is active. |
| `no-matching-briefings` | Rendered when briefings exist but the current filters narrow the list to zero. Includes a "Clear filters" button (`clear-briefing-filters`). |
| `briefing-card-{id}` | A single briefing row |
| `briefing-audio-{id}` | The custom audio player container for a completed audio briefing. A focusable `role="region"` wrapping the play/pause button, scrubber, time readout, speed selector, and download link. The actual `<audio>` element is mounted inside but is not the hook target. |
| `briefing-image-{id}` | The `<img>` element for a completed infographic briefing |
| `briefing-video-{id}` | The `<video>` element for a completed video briefing |
| `briefing-text-{id}` | The rendered text content for a text briefing |
...
| `user-resume-text` | Full résumé text (Textarea) — feeds the Resume Critique briefing |
...
| `toggle-briefings-feature` | Master toggle for NotebookLM briefings (in Appearance → AI features). When off the DailyBriefingStrip on the dashboard, the Briefings TopNav entry, the NotebookLM auth + auto-briefing sub-sections in Settings, and the per-job Briefings dropdown on applied cards all hide. The `/briefings` route stays reachable by direct URL so any already-generated briefings remain viewable. Read the toggle state from `useAppearance().value.briefings`. |

### Briefings generation form (on `/briefings`)

| Hook | Form element |
| ---- | ------------ |
| `briefing-type` | Briefing-type selector (Select) — the dropdown of all available output types |
| `briefing-target-job` | Target-job selector (Select) — only shown for per-job briefing types (interview prep, pre-application brief, flashcards, quiz) |

### Briefings inbox filters (on `/briefings`)

Filters are purely client-side — the underlying `briefings.list` tRPC query still returns the full set; the filter bar narrows what's rendered.

| Hook | Form element |
| ---- | ------------ |
| `briefing-search` | Search-by-title text input. Case-insensitive `includes` match against `briefing.title`. |
| `briefing-filter-type` | Filter by briefing type (Select). `all` shows every type. |
| `briefing-filter-status` | Filter by status (Select). One of `all`, `pending`, `generating`, `complete`, `failed`. |
| `briefing-sort` | Sort order (Select). One of `newest` (default), `oldest`, `type`. |
...
| `auto-daily-briefing` | Toggle the daily NotebookLM coach-briefing schedule (Switch) |
| `auto-weekly-briefing` | Toggle the weekly NotebookLM market-pulse schedule (Switch) |
