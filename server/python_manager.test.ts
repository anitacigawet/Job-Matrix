...
describe("Python runtime selection", () => {
  it("accepts the Windows launcher default Python 3 when it is 3.10 or newer", () => {
    ...
    expect(resolveSystemPython({ PATH: "C:\\Windows" }, "win32")).toEqual({
      command: "py",
      args: ["-3"],
    });
  });

  it("uses Unix Python candidates on Linux", () => {
    delete process.env.PYTHON_BIN;
    execFileSyncMock.mockImplementation((command: string) => {
      if (command === "python3") return Buffer.from("");
      throw new Error("not installed");
    });

    expect(resolveSystemPython({ PATH: "/usr/bin" }, "linux")).toEqual({
      command: "python3",
      args: [],
    });
  });
