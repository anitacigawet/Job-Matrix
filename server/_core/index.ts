...
import { requireLoopbackHost, requireSameOriginTrpc } from "./request-security";
...
async function startServer() {
  ...
  app.use(requireLoopbackHost(ENV.port));
  // Reject cross-origin, cross-site, and non-JSON API requests before either
  // body parser allocates memory for attacker-controlled form data.
  ...
  if (process.env.NODE_ENV === "development") {
    await setupVite(app, server, ENV.port);
  } else {
    serveStatic(app);
  }
