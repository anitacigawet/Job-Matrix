...
const CREDENTIALED_SOURCE_IDS = ["adzuna", "usajobs", "jooble", "themuse"] as const;
...
function getDataSourcesForApi(): { sources: DataSourceDescriptor[] } {
  ...
  // ── Adzuna ────────────────────────────────────────────────
  const adzunaAppId = ENV.adzunaAppId || ds.adzuna?.appId || settings.adzunaAppId || "";
  const adzunaAppKey = ENV.adzunaAppKey || ds.adzuna?.appKey || settings.adzunaAppKey || "";
  const adzunaSourceOrigin: DataSourceDescriptor["source"] = (ENV.adzunaAppId && ENV.adzunaAppKey)
    ? "env"
    : (adzunaAppId && adzunaAppKey ? "settings" : "none");

  // ── USAJobs ───────────────────────────────────────────────
  const usajobsEmail = ENV.usajobsEmail || ds.usajobs?.email || "";
  const usajobsApiKey = ENV.usajobsApiKey || ds.usajobs?.apiKey || "";
  const usajobsSourceOrigin: DataSourceDescriptor["source"] = (ENV.usajobsEmail && ENV.usajobsApiKey)
    ? "env"
    : (usajobsEmail && usajobsApiKey ? "settings" : "none");
    ...
    sources: [
      {
        ...
        fields: [
          { name: "appId", label: "App ID", inputType: "text", placeholder: "c17cfb68", valueMasked: maskCredential(adzunaAppId), required: true },
          { name: "appKey", label: "App Key", inputType: "password", placeholder: "••••••••", valueMasked: maskCredential(adzunaAppKey), required: true },
        ],
      },
      {
        id: "usajobs",
        ...
        noAuthRequired: false,
        fields: [
          { name: "email", label: "Email (User-Agent)", inputType: "email", placeholder: "you@example.com", valueMasked: maskCredential(usajobsEmail), required: true },
          { name: "apiKey", label: "API Key", inputType: "password", placeholder: "••••••••", valueMasked: maskCredential(usajobsApiKey), required: true },
        ],
      },
      {
        id: "jooble",
        ...
        noAuthRequired: false,
        fields: [
          { name: "apiKey", label: "API Key (partner)", inputType: "password", placeholder: "••••••••", valueMasked: maskCredential(joobleApiKey), required: true },
        ],
      },
      {
        id: "themuse",
        ...
        noAuthRequired: true,
        fields: [
          { name: "apiKey", label: "API Key (optional — raises rate limit)", inputType: "password", placeholder: "leave blank for no-auth tier", valueMasked: maskCredential(themuseApiKey), required: false },
        ],
      },
      {
        id: "remotive",
...
export const settingsRouter = router({
  ...
  updateNotifications: protectedProcedure
    .input(z.object({
      ...
      notifyDigestFrequency: z.enum(["immediate", "daily", "weekly", "never"]).optional(),
    }))
    .mutation(async ({ ctx, input }) => {
      ...
      if (input.notificationsEnabled !== undefined) updates.notificationsEnabled = input.notificationsEnabled ? 1 : 0;
      if (input.notifyOnNewEligible !== undefined) updates.notifyOnNewEligible = input.notifyOnNewEligible ? 1 : 0;
      if (input.notifyOnScanComplete !== undefined) updates.notifyOnScanComplete = input.notifyOnScanComplete ? 1 : 0;
      if (input.notifyDigestFrequency !== undefined) updates.notifyDigestFrequency = input.notifyDigestFrequency;
  ...
  updateAutoScan: protectedProcedure
    .input(z.object({
      ...
      autoScanFrequency: z.enum(["every_6h", "every_12h", "daily", "every_2d", "weekly"]).optional(),
      autoScanIncludeAI: z.boolean().optional(),
      autoDailyBriefing: z.boolean().optional(),
      autoWeeklyBriefing: z.boolean().optional(),
    }))
    .mutation(async ({ ctx, input }) => {
      const db = await getDb();
      if (!db) throw new Error("Database not available");
      ...
      const updates: Record<string, any> = {};
      if (input.autoScanEnabled !== undefined) updates.autoScanEnabled = input.autoScanEnabled ? 1 : 0;
      if (input.autoScanFrequency !== undefined) updates.autoScanFrequency = input.autoScanFrequency;
      if (input.autoScanIncludeAI !== undefined) updates.autoScanIncludeAI = input.autoScanIncludeAI ? 1 : 0;
      if (input.autoDailyBriefing !== undefined) updates.autoDailyBriefing = input.autoDailyBriefing ? 1 : 0;
      if (input.autoWeeklyBriefing !== undefined) updates.autoWeeklyBriefing = input.autoWeeklyBriefing ? 1 : 0;

      if (input.autoScanEnabled === true) {
        const freq = input.autoScanFrequency || existing?.autoScanFrequency || "daily";
  ...
  testProvider: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      const { testProviderConnection } = await import("./services/provider-tester");
  ...
  saveDataSource: publicProcedure
    ...
    .mutation(({ input }) => {
      ...
      if (input.source === "adzuna") {
        ...
        if (input.fields.appId) current.dataSources.adzuna.appId = input.fields.appId;
        if (input.fields.appKey) current.dataSources.adzuna.appKey = input.fields.appKey;
      } else if (input.source === "usajobs") {
        current.dataSources.usajobs = current.dataSources.usajobs ?? {};
        if (input.fields.email) current.dataSources.usajobs.email = input.fields.email;
        if (input.fields.apiKey) current.dataSources.usajobs.apiKey = input.fields.apiKey;
      } else if (input.source === "jooble") {
        current.dataSources.jooble = current.dataSources.jooble ?? {};
        if (input.fields.apiKey) current.dataSources.jooble.apiKey = input.fields.apiKey;
      } else if (input.source === "themuse") {
        current.dataSources.themuse = current.dataSources.themuse ?? {};
        if (input.fields.apiKey) current.dataSources.themuse.apiKey = input.fields.apiKey;
      }
  ...
  testDataSource: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      const { testDataSourceConnection } = await import("./services/data-source-tester");
      const settings = readSettings();
      const ds = settings.dataSources ?? {};
      const typed = input.fields ?? {};
      const typedHas = (k: string) => (typed[k]?.trim().length ?? 0) > 0;
      if (input.source === "adzuna") {
        const appId = typedHas("appId") ? typed.appId.trim() : (ENV.adzunaAppId || ds.adzuna?.appId || settings.adzunaAppId || "");
        const appKey = typedHas("appKey") ? typed.appKey.trim() : (ENV.adzunaAppKey || ds.adzuna?.appKey || settings.adzunaAppKey || "");
        if (!appId || !appKey) return { ok: false, message: "No Adzuna credentials available. Enter app_id + app_key and Save first.", latencyMs: 0 };
        return testDataSourceConnection("adzuna", { appId, appKey });
      }
      if (input.source === "usajobs") {
        const email = typedHas("email") ? typed.email.trim() : (ENV.usajobsEmail || ds.usajobs?.email || "");
        const apiKey = typedHas("apiKey") ? typed.apiKey.trim() : (ENV.usajobsApiKey || ds.usajobs?.apiKey || "");
        if (!email || !apiKey) return { ok: false, message: "No USAJobs credentials. Enter email + API key and Save first.", latencyMs: 0 };
        return testDataSourceConnection("usajobs", { email, apiKey });
      }
      if (input.source === "jooble") {
        const apiKey = typedHas("apiKey") ? typed.apiKey.trim() : (ENV.joobleApiKey || ds.jooble?.apiKey || "");
        if (!apiKey) return { ok: false, message: "No Jooble API key. Enter your partner key and Save first.", latencyMs: 0 };
        return testDataSourceConnection("jooble", { apiKey });
      }
      if (input.source === "themuse") {
        const apiKey = typedHas("apiKey") ? typed.apiKey.trim() : (ENV.themuseApiKey || ds.themuse?.apiKey || "");
      ...
      return { ok: false, message: `Unknown source: ${input.source}`, latencyMs: 0 };
    }),
  ...
  updatePlatforms: protectedProcedure
    .input(z.object({
      enabledPlatforms: z.array(z.enum(["indeed", "glassdoor", "linkedin", "ziprecruiter", "google", "adzuna"])).min(1),
    }))
    .mutation(async ({ ctx, input }) => {
      const db = await getDb();
      if (!db) throw new Error("Database not available");
...
function calculateNextRun(frequency: string): Date {
  ...
  switch (frequency) {
    ...
    case "every_2d":  return new Date(now.getTime() + 48 * 60 * 60 * 1000);
