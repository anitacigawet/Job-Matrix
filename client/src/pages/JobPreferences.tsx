...
import { Loader2, Save, MapPin, GraduationCap, Clock, Briefcase, Wrench, DollarSign, Sparkles, CheckCircle2, AlertCircle, Settings, KeyRound, X, FileText } from "lucide-react";
...
export function JobPreferences() {
  ...
  const { data: profile, isLoading: profileLoading } = trpc.onboarding.getProfile.useQuery();
  ...
  const saveProfileMutation = trpc.onboarding.saveProfile.useMutation({
    onSuccess: () => {
      ...
      setTimeout(() => {
        setProfileSavedAt((prev) => (prev && Date.now() - prev >= 4000 ? null : prev));
      }, 4000);
  ...
  const updateTitleStatusMutation = trpc.onboarding.updateJobTitleStatus.useMutation({
    onSuccess: () => {
  ...
  const handleAddTitle = () => {
    ...
    saveTitlesMutation.mutate({
      titles: [...jobTitles.map(t => ({ title: t.title, isActive: t.isActive })), { title, isActive: true }],
    });
  ...
  const handleQuickFillApply = (newTitles: string[]) => {
    const existingLower = new Set(jobTitles.map(t => t.title.toLowerCase().trim()));
    const fresh = newTitles.filter(t => !existingLower.has(t.toLowerCase().trim()));
    if (fresh.length === 0) {
    ...
    toast.success(`Added ${fresh.length} title${fresh.length === 1 ? "" : "s"} to your list`);
  };
  ...
  const handleSaveNewTitles = () => {
    ...
    saveTitlesMutation.mutate({
      titles: generateMutation.data.titles.map(title => ({ title, isActive: true })),
    });
  };

  // Computed values
  const activeTitles = useMemo(() => jobTitles.filter(t => t.isActive), [jobTitles]);
  const inactiveTitles = useMemo(() => jobTitles.filter(t => !t.isActive), [jobTitles]);
    ...
    <div className="min-h-screen bg-background">
      <div className="container max-w-4xl mx-auto py-8">
        ...
        <div className="space-y-6">
          ...
          {/* ============================================================ */}
          <Card className="glass-card" data-agent-status="location-work-preferences">
            <CardHeader>
            ...
            <CardContent className="space-y-5">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Select value={state} onValueChange={(v) => { setState(v); setProfileDirty(true); }}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    max={500}
                    value={searchRadius}
                    onChange={(e) => { setSearchRadius(Number(e.target.value)); setProfileDirty(true); }}
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-3 pt-2">
                    <Switch
                      ...
                      onCheckedChange={(v) => { setWillingToRelocate(v); setProfileDirty(true); }}
              ...
              <div className="space-y-2">
                ...
                <Select value={remotePreference} onValueChange={(v) => { setRemotePreference(v); setProfileDirty(true); }}>
            ...
            <CardContent className="space-y-5">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Select value={educationLevel} onValueChange={(v) => { setEducationLevel(v); setProfileDirty(true); }}>
                ...
                <div className="space-y-2">
                  <Label className="text-sm font-medium">Total Years of Career Experience</Label>
                  <Select value={yearsExperience} onValueChange={(v) => { setYearsExperience(v); setProfileDirty(true); }}>
          ...
          {/* ============================================================ */}
          <Card className="glass-card" data-agent-status="skills-experience-preferences">
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
            ...
            <CardContent className="space-y-4">
              <Textarea
                ...
                value={skillsRaw}
                onChange={(e) => { setSkillsRaw(e.target.value); setProfileDirty(true); }}
              ...
              {profile?.skillsParsed && Array.isArray(profile.skillsParsed) && profile.skillsParsed.length > 0 && (
                <div className="bg-green-500/10 border border-green-500/30 rounded-lg p-4" data-agent-status="parsed-skills-list">
                  <p className="text-xs font-medium text-green-400 mb-2">Currently parsed skills:</p>
                  <div className="flex flex-wrap gap-2">
                    {profile.skillsParsed.map((s: any, i: number) => (
                      <Badge key={i} variant="secondary" className="bg-green-500/20 text-green-300 border-green-500/40">
          ...
          {activeTab === "resume" && (<>
          {/* ============================================================ */}
          {/* SECTION 3.5: Résumé text (powers the Resume Critique briefing) */}
          {/* Promoted out of Skills & Experience in D11.16a — was buried and  */}
          {/* therefore the Resume Critique feature was undiscoverable.        */}
          {/* ============================================================ */}
          <Card className="glass-card" data-agent-status="resume-text-section">
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <FileText className="h-5 w-5 text-cyan-400" />
                Résumé Text
                <Badge variant="outline" className="text-xs bg-cyan-500/10 text-cyan-400 border-cyan-500/40">Optional</Badge>
              </CardTitle>
              <CardDescription>
                Required only if you want to generate the Resume Critique briefing on the Briefings page. Stays on your machine — only sent to NotebookLM when you explicitly run the critique.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-2">
              <Textarea
                ...
                value={resumeText}
                onChange={(e) => { setResumeText(e.target.value); setProfileDirty(true); }}
                data-agent-input="user-resume-text"
                rows={10}
                className="resize-y font-mono text-xs"
              />
              <p className="text-xs text-muted-foreground">
                Sent only when you click <span className="text-foreground font-medium">Generate</span> on a Resume Critique briefing — nowhere else.
              </p>
            </CardContent>
          </Card>
          </>)}
          ...
          {/* ============================================================ */}
          <Card className="glass-card" data-agent-status="salary-preferences">
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <DollarSign className="h-5 w-5 text-yellow-400" />
                Salary Preferences
                <Badge variant="outline" className="text-xs bg-yellow-500/10 text-yellow-400 border-yellow-500/40">Soft Filter</Badge>
              </CardTitle>
              <CardDescription>
                Optional — jobs without salary info will NOT be rejected. This only flags jobs that explicitly list a salary below your minimum.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="flex items-center gap-3">
                <Switch
                  ...
                  onCheckedChange={(v) => { setSalaryFilterEnabled(v); setProfileDirty(true); }}
                  data-agent-input="user-salary-filter-enabled"
                />
                <span className="text-sm text-muted-foreground">
                  {salaryFilterEnabled ? "Salary filter enabled" : "Salary filter disabled (all jobs pass)"}
                </span>
              </div>

              {salaryFilterEnabled && (
                <div className="space-y-2">
                  <Label className="text-sm font-medium">Minimum Annual Salary ($)</Label>
                  <Input
                    type="number"
                    min={0}
                    step={1000}
                    ...
                    value={minSalary}
                    onChange={(e) => { setMinSalary(e.target.value); setProfileDirty(true); }}
                  ...
                  <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-lg p-3">
                    <div className="flex items-start gap-2">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Many job listings don't include salary information. This filter will only reject jobs that <strong>explicitly list</strong> a salary below your minimum. Jobs with no salary data will always pass through.
                      </p>
                    </div>
                  </div>
                </div>
              ...
              >
                {saveProfileMutation.isPending ? (
                  ...
                  <>
                    ...
                    {profileDirty && <span className="ml-2 text-xs opacity-75">(unsaved changes)</span>}
                  </>
                )}
              </Button>
              {profileSavedAt && (
          ...
          {/* ============================================================ */}
          <Card className="glass-card" data-agent-status="job-titles-management">
            <CardHeader>
              <div className="flex items-start justify-between gap-3 flex-wrap">
                <CardTitle className="flex items-center gap-2">
                  <Briefcase className="h-5 w-5 text-blue-400" />
                  Manage Job Titles ({activeTitles.length} active / {jobTitles.length} total)
                </CardTitle>
                <Button
                  variant="outline"
                  size="sm"
            ...
            <CardContent className="space-y-4">
              ...
              {jobTitles.length > 0 ? (
                <div className="space-y-2">
                  {jobTitles.map((t) => (
                    ...
                    >
                      <span className={`font-medium text-sm ${t.isActive ? "text-foreground" : "text-muted-foreground line-through"}`}>
                        {t.title}
                      </span>
                      <Switch
                        checked={t.isActive}
                        onCheckedChange={() => handleToggleTitle(t.id, t.isActive)}
              ...
              <div className="space-y-3">
                <Label className="text-sm font-medium">Manually Add or Generate Job Titles</Label>
                <p className="text-xs text-muted-foreground">
                  Type a specific title to add it manually, or enter a category and click Generate for AI variations.
                </p>
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    disabled={!newJobType.trim() || generateMutation.isPending || saveTitlesMutation.isPending}
                    variant="secondary"
                  ...
                  <Button
                    ...
                    disabled={!newJobType.trim() || generateMutation.isPending}
                  ...
                  <div className="space-y-3">
                    <div className="bg-purple-500/10 border border-purple-500/30 rounded-lg p-4">
                      <p className="text-sm font-medium text-purple-400 mb-3">
                        AI Suggested {generateMutation.data.titles.length} titles:
                      </p>
                      <div className="flex flex-wrap gap-2 mb-4">
                        {generateMutation.data.titles.map((title, i) => (
                          <Badge key={i} variant="secondary" className="bg-purple-500/20 text-purple-300 border-purple-500/40">
                            {title}
                          </Badge>
                        ))}
                      </div>
