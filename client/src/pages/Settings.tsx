...
import {
  Loader2, Bell, BellOff, Clock, Play, Send,
  Settings as SettingsIcon, Zap, CalendarClock, CheckCircle2,
  KeyRound, ExternalLink, Trash2,
  DatabaseBackup, ShieldAlert,
} from "lucide-react";
import { toast } from "sonner";
import { getFriendlyApiErrorMessage } from "@/lib/api-errors";
import { NotebookLMAuthCard } from "@/components/NotebookLMAuthCard";
...
const PROVIDER_META: Record<ProviderId, {
  ...
  gemini: {
    ...
    modelHint: "Default is gemini-2.0-flash. Other options include gemini-2.0-pro-exp-02-05 and gemini-1.5-flash-001.",
    rateLimitHint: "Recommended: 1.0 RPS for the free tier; raise it for paid keys.",
  },
  openai: {
    label: "OpenAI",
    keyHelpText: "Get a key at",
    keyHref: "https://platform.openai.com/api-keys",
    modelHint: "Default is gpt-4o-mini. Other options include gpt-4o and gpt-4.1-mini.",
    rateLimitHint: "Recommended: 1.0 RPS for low tiers; raise it for higher org limits.",
  },
  deepseek: {
    label: "DeepSeek",
    keyHelpText: "Get a key at",
    keyHref: "https://platform.deepseek.com/api_keys",
    modelHint: "Default is deepseek-chat. You can also use deepseek-reasoner for harder filtering.",
    rateLimitHint: "Recommended: 1.0 RPS for DeepSeek Free/Tier 1; higher for paid tiers.",
  },
};
...
export function SettingsPage() {
  ...
  const briefingsVisible = appearance.briefings;
  ...
  const [autoDailyBriefing, setAutoDailyBriefing] = useState(false);
  const [autoWeeklyBriefing, setAutoWeeklyBriefing] = useState(false);
  ...
  useEffect(() => {
    if (settings && !initialized) {
      ...
      setAutoDailyBriefing(!!(settings as any).autoDailyBriefing);
      setAutoWeeklyBriefing(!!(settings as any).autoWeeklyBriefing);
      setInitialized(true);
    }
  ...
  const testNotifMutation = trpc.settings.sendTestNotification.useMutation({
    onSuccess: data => {
      if (data.success) toast.success("Test notification sent! Check your server console.");
  ...
  const [keyInputs, setKeyInputs] = useState<Record<ProviderId, string>>({ gemini: "", openai: "", deepseek: "" });
  const [modelInputs, setModelInputs] = useState<Record<ProviderId, string>>({ gemini: "", openai: "", deepseek: "" });
  ...
  useEffect(() => {
    if (llmQuery.data && !llmInitialized) {
      ...
      const m: Record<ProviderId, string> = { gemini: "", openai: "", deepseek: "" };
      for (const p of llmQuery.data.providers) m[p.id as ProviderId] = p.model ?? "";
      setModelInputs(m);
  ...
  const isTesting = testProviderMutation.isPending || testSavedProviderMutation.isPending;

  const providersById = useMemo(() => {
    const out: Partial<Record<ProviderId, NonNullable<typeof llmQuery.data>["providers"][number]>> = {};
  ...
  const handleSaveNotifications = () => {
    updateNotifMutation.mutate({
      ...
      notifyDigestFrequency: digestFrequency as "immediate" | "daily" | "weekly" | "never",
    });
  };
  ...
  const handleSaveAutoScan = () => {
    updateAutoScanMutation.mutate({
      autoScanEnabled,
      autoScanFrequency: autoScanFrequency as "every_6h" | "every_12h" | "daily" | "every_2d" | "weekly",
      autoScanIncludeAI,
      autoDailyBriefing,
      autoWeeklyBriefing,
    });
  ...
  const nextRunFormatted = settings?.autoScanNextRun ? new Date(settings.autoScanNextRun).toLocaleString() : "Not scheduled";
  const lastRunFormatted = settings?.autoScanLastRun ? new Date(settings.autoScanLastRun).toLocaleString() : "Never";
    ...
    <div className="min-h-screen bg-background">
      <div className="container max-w-3xl mx-auto py-8">
        ...
        <div className="space-y-6">
          ...
          {activeTab === "llm" && (
          <div className="space-y-6">
            <Card className="glass-card" data-agent-status="llm-settings-card">
              <CardHeader>
                <CardTitle className="flex items-center gap-2 flex-wrap">
                  ...
                  {activeHasKey && (
                    <Button
                      ...
                      onClick={() => testSavedProviderMutation.mutate({ provider: activeProvider })}
                      disabled={isTesting}
                ...
                <CardDescription>
                  Job Matrix uses an LLM to filter and score job listings. Pick your provider and bring your own API key — it is stored on your machine and sent only to that provider for authentication.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Only the active provider's key is used for AI calls. The other keys remain saved for quick switching — switch tabs below to edit a non-active provider's key without changing which one runs.
                  </p>
                </div>

                <Tabs value={editingProvider} onValueChange={(v) => setEditingProvider(v as ProviderId)} className="space-y-4">
                  <TabsList className="grid w-full grid-cols-3">
                    {PROVIDER_TAB_ORDER.map((p) => (
                      <TabsTrigger key={p} value={p} data-agent-action={`edit-${p}-provider`}>{PROVIDER_META[p].label}</TabsTrigger>
                    ))}
                  </TabsList>
                  ...
                  {PROVIDER_TAB_ORDER.map((p) => {
                    ...
                    return (
                      <TabsContent key={p} value={p} className="space-y-5">
                        {info?.hasKey && (
                          <div className="p-3 rounded-lg border border-green-500/30 bg-green-500/5 text-sm" aria-live="polite" data-agent-status={`active-${p}-key`}>
                            Saved key: <code className="font-mono">{info.keyMasked}</code>{" "}
                            <span className="text-muted-foreground">
                              ({info.keySource === "env" ? "from env var" : "from settings file"})
                            </span>
                          </div>
                        )}
                        ...
                        <div className="space-y-2">
                          ...
                          <Input
                            type="password"
                            placeholder={p === "openai" ? "sk-..." : p === "deepseek" ? "sk-..." : "AIzaSy..."}
                            value={keyInputs[p]}
                          ...
                          <div className="flex items-center justify-between gap-2 flex-wrap">
                            <p className="text-xs text-muted-foreground flex-1 min-w-0">
                              {meta.keyHelpText}{" "}
                              <a href={meta.keyHref} target="_blank" rel="noopener noreferrer" className="text-purple-400 hover:underline inline-flex items-center gap-1">
                                {new URL(meta.keyHref).host} <ExternalLink className="h-3 w-3" />
                              </a>
                              . The key is stored in <code>./data/settings.json</code>.
                            </p>
                            <Button
                              variant="default"
                              size="sm"
                              ...
                              onClick={() => {
                                ...
                                if (!typedKey && info?.hasKey) {
                                  ...
                                  testSavedProviderMutation.mutate({ provider: p });
                                  return;
                                }
                                testProviderMutation.mutate({
                                  provider: p,
                                  apiKey: typedKey,
                                  model: typedModel || info?.defaultModel || "",
                                });
                              }}
                              disabled={isTesting}
                              className="bg-purple-500/80 hover:bg-purple-500 shrink-0"
                              title={keyInputs[p].trim() ? "Test the typed key (not saved yet)" : "Test the saved key"}
                            >
                              {isTesting ? (
                        ...
                        <div className="space-y-2">
                          ...
                          <Input
                            ...
                            value={modelInputs[p]}
                            onChange={(e) => setModelInputs((s) => ({ ...s, [p]: e.target.value }))}
                        ...
                        <div className="flex justify-end">
                          <Button
                            ...
                            onClick={() => llmClearMutation.mutate({ provider: p })}
                            disabled={llmClearMutation.isPending || !info?.hasKey || info?.keySource === "env"}
                          >
                ...
                <div className="space-y-4 pt-2 border-t border-border/40">
                  <div className="flex items-center space-x-2">
                    ...
                    />
                    <Label htmlFor="rateLimitEnabled" className="text-sm font-medium cursor-pointer">Enable Rate Limiting</Label>
                  </div>

                  {rateLimitEnabled && (
                    <div className="space-y-2 pl-6 animate-in slide-in-from-top-1 duration-200">
                      <Label htmlFor="rateLimitRps" className="text-xs text-muted-foreground">Requests per second (RPS)</Label>
                      <Input
                        id="rateLimitRps"
                        type="number"
                        min="0.1"
                        step="0.1"
                        value={rateLimitRps}
                        onChange={(e) => setRateLimitRps(parseFloat(e.target.value) || 1)}
          ...
          {activeTab === "notif" && (
          <div className="space-y-6">
            <Card className="glass-card" data-agent-status="notifications-settings-card">
              <CardHeader>
                ...
                <CardDescription>
                  Get notified when new eligible jobs are found or scans complete.
                  Notifications are logged to the server console (local-first).
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-5">
                <div className="flex items-center justify-between p-4 rounded-lg border border-border/50 bg-muted/20">
                  <div className="flex items-center gap-3">
                    {notificationsEnabled ? <Bell className="h-5 w-5 text-blue-400" /> : <BellOff className="h-5 w-5 text-muted-foreground" />}
                    <div>
                ...
                {notificationsEnabled && (
                  <>
                    ...
                    <div className="space-y-3">
                      <Label className="text-sm font-medium text-muted-foreground">Notification Events</Label>
                      <div className="flex items-center justify-between p-3 rounded-lg border border-border/30">
                        <div>
                          <Label className="text-sm font-medium">New Eligible Jobs</Label>
                          <p className="text-xs text-muted-foreground">Notify when AI job filtering finds new jobs matching your profile</p>
                        </div>
                        <Switch checked={notifyOnNewEligible} onCheckedChange={setNotifyOnNewEligible} />
                      </div>
                      <div className="flex items-center justify-between p-3 rounded-lg border border-border/30">
                        <div>
                          <Label className="text-sm font-medium">Scan Complete</Label>
                          <p className="text-xs text-muted-foreground">Notify when a Global Search or AI Job Filtering finishes</p>
                        </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-xs text-muted-foreground">How often to receive notification summaries</p>
                    </div>
                  </>
                )}
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <Button
                    ...
                    disabled={testNotifMutation.isPending || !notificationsEnabled}
                  >
          ...
          {activeTab === "scan" && (
          <div className="space-y-6">
            <Card className="glass-card" data-agent-status="autoscan-settings-card">
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                ...
                <CardDescription>
                  Automatically run Global Search and AI Job Filtering on a schedule so you don't have to click buttons manually.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-5">
                <div className="flex items-center justify-between p-4 rounded-lg border border-border/50 bg-muted/20">
                  <div className="flex items-center gap-3">
                    {autoScanEnabled ? <Zap className="h-5 w-5 text-purple-400" /> : <Clock className="h-5 w-5 text-muted-foreground" />}
                    <div>
                      <Label className="text-sm font-medium">Enable Auto-Scan</Label>
                      <p className="text-xs text-muted-foreground">Automatically search for new jobs on a schedule</p>
                    </div>
                  </div>
                ...
                {autoScanEnabled && (
                  <>
                    ...
                    <div className="space-y-2">
                      ...
                      <Select value={autoScanFrequency} onValueChange={setAutoScanFrequency}>
                        <SelectTrigger><SelectValue /></SelectTrigger>
                        <SelectContent>
                      ...
                      <p className="text-xs text-muted-foreground">How often to automatically search for new jobs</p>
                    </div>
                    ...
                    <div className="flex items-center justify-between p-3 rounded-lg border border-border/30">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <Label className="text-sm font-medium">Include AI Job Filtering</Label>
                          <p className="text-xs text-muted-foreground">Automatically run AI filtering after each scan to identify eligible jobs</p>
                        </div>
                      </div>
                      <Switch checked={autoScanIncludeAI} onCheckedChange={setAutoScanIncludeAI} />
                    </div>
                        ...
                        <div className="space-y-3">
                          <div>
                            <Label className="text-sm font-semibold text-purple-300">
                              NotebookLM briefings (auto-generated)
                            </Label>
                            <p className="text-xs text-muted-foreground mt-0.5">
                              Each generation consumes NotebookLM quota — leave off unless you've connected on the Settings card above.
                            </p>
                          </div>
                          <div className="flex items-center justify-between p-3 rounded-lg border border-border/30">
                            <div className="flex items-center gap-3">
                              <CalendarClock className="h-5 w-5 text-purple-400" />
                              <div>
                                <Label className="text-sm font-medium">Daily Coach Briefing</Label>
                                <p className="text-xs text-muted-foreground">Two-host audio briefing once every 24h. View in /briefings.</p>
                              </div>
                            </div>
                            <Switch
                              checked={autoDailyBriefing}
                              onCheckedChange={setAutoDailyBriefing}
                              data-agent-input="auto-daily-briefing"
                            />
                          </div>
                          <div className="flex items-center justify-between p-3 rounded-lg border border-border/30">
                            <div className="flex items-center gap-3">
                              <CalendarClock className="h-5 w-5 text-purple-400" />
                              <div>
                                <Label className="text-sm font-medium">Weekly Market Pulse</Label>
                                <p className="text-xs text-muted-foreground">Tight ~5-min market-summary audio once a week.</p>
                              </div>
                            </div>
                            <Switch
                              checked={autoWeeklyBriefing}
                              onCheckedChange={setAutoWeeklyBriefing}
                              data-agent-input="auto-weekly-briefing"
                            />
                          </div>
                        </div>
                      </>
                    ...
                    <Separator />
                    <div className="grid grid-cols-2 gap-4" aria-live="polite" data-agent-status="autoscan-schedule">
                      <div className="p-3 rounded-lg bg-muted/20 border border-border/30">
                        <p className="text-xs text-muted-foreground mb-1">Last Run</p>
                        <p className="text-sm font-medium">{lastRunFormatted}</p>
                      </div>
                      <div className="p-3 rounded-lg bg-muted/20 border border-border/30">
                        <p className="text-xs text-muted-foreground mb-1">Next Scheduled</p>
                        <p className="text-sm font-medium text-purple-400">{nextRunFormatted}</p>
                      </div>
                    </div>
                  </>
                )}
                ...
                {!autoScanEnabled && (
                  <div className="bg-muted/20 border border-border/30 rounded-lg p-4 text-center">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Enable auto-scan to have the system automatically search for new jobs and run AI job filtering on your behalf.
                      You can still run manual scans from the dashboard at any time.
                    </p>
                  </div>
                )}
              </CardContent>
          ...
          {activeTab === "notebook" && briefingsVisible && <NotebookLMAuthCard />}
          ...
          {activeTab === "local-data" && (
            <Card className="glass-card border-red-500/20">
              <CardHeader>
                ...
                <CardDescription>
                  Return Job Matrix to first-run setup on this computer. This does not delete the
                  application itself or affect any external account.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="flex items-start gap-3 rounded-lg border border-amber-500/30 bg-amber-500/10 p-4">
                  ...
                  <p className="text-sm text-muted-foreground">
                    This permanently clears your profile, saved jobs, scan history, presets, and
                    application records from the local SQLite database. Export anything you want
          ...
          <Dialog open={resetDialogOpen} onOpenChange={setResetDialogOpen}>
            <DialogContent className="max-w-md border-red-500/50 bg-background/95 backdrop-blur-xl">
              <DialogHeader>
                <DialogTitle className="text-red-400">Reset all local Job Matrix data?</DialogTitle>
                <DialogDescription>
                  You will return to first-run setup. Job Matrix cannot undo this action.
                </DialogDescription>
              </DialogHeader>
